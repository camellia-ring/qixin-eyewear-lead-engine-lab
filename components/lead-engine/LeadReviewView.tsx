...
export function LeadReviewView({ state }: { state: LeadEngineState }) {
  const {
    ...
    specialFilter, statusFilter, typeFilters, completionAnchor, completionPeriod,
  } = state;
  ...
  return <>
    <section className={[styles.reviewWorkspace, styles.workspaceExpanded].join(" ")} aria-label={VIEW_LABELS.review}>
      ...
      <div className={styles.tableFrame} aria-busy={loading || leadLoading}>
        ...
        {!loading && !leadLoading && !leadPage.rows.length ? <div className={styles.emptyState}><IconSearch size={28} /><b>{statusFilter === "needs_review" && rejectedCount ? "当前时间与客户范围暂无待审核客户" : "当前筛选没有客户"}</b><p>{statusFilter === "needs_review" && rejectedCount ? `当前时间范围有 ${rejectedCount} 家已淘汰客户，可查看未通过原因。` : "调整完成时间、客户范围或清除筛选后再查看。"}</p>{statusFilter === "needs_review" && rejectedCount ? <button type="button" onClick={() => { state.setStatusFilter("rejected"); state.setPage(1); }}>查看 {rejectedCount} 家已淘汰客户</button> : <button type="button" onClick={state.clearFilters}>清除筛选</button>}</div> : null}
