...
export function AutomaticDiscoveryView({ state }: { state: LeadEngineState }) {
  ...
  return <section className={styles.toolWorkspace} aria-label={VIEW_LABELS.discovery}>
    ...
    <section className={styles.engineDashboard}>
      ...
      <div className={styles.engineExports}><span>自动合格客户可在审核台按地区、国家、客户类型和产品分类查看；人工批准后会受控移交到网站 CRM。</span><button className={styles.secondaryButton} type="button" disabled={!totalApprovedCount} onClick={() => state.setActiveView("export")}><IconFileExport size={17} />前往 CRM 导出</button></div>
    </section>
    ...
    <div className={styles.discoveryPrinciples}><article><b>预置官方来源免费运行</b><span>付费搜索和 GPT provider 有接口但默认关闭、无密钥也可运行</span></article><article><b>每批领取 20 个候选</b><span>公司级并发从 3 自动升至 5；限流、超时或错误升高时降至 2 或 1</span></article><article><b>人工批准仍是联系闸门</b><span>自动合格客户进入待审核；仅批准后进入 CRM，系统不自动发送邮件</span></article></div>
