...
2026-08-23 的无 API 容量复核启用了三个可公开核验的真实来源：MIDO 2026 服务端展商地图、100% Optical 展商详情和 Neo Tokyo 2026 公开 JSON 目录。现场解析得到 713、230 和 72 个候选，共 1,015 个；这是进入逐家官网核验的候选容量，不是合格公司数量。DIOPS、OPTYKA、PSO 和 OXO 的当前页面只解析出展会导航或服务商链接，已停用并保留原因，不能用它们填充每日目标。
...
已确认容量较大的来源在 `parserConfigJson` 中设置 `repeatDuringDay: true`。它们仍然每批最多处理 5 家，只是在一个运行日内按持久化 `offset` 每 15 分钟继续下一批；普通 daily 来源仍然每天最多运行一次。到达目录末尾后统一归零游标并延后 7 天。
